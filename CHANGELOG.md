...
- **Config** — new top-level `skillHub` key (11 fields: `enabled` false, `syncIntervalMs` 86400000, `bootstrapUrl` skills-library.com, `searchFallbackUrl`/`downloadBaseUrl` skills.sh, `embedBaseUrl` 127.0.0.1:3114/v1, `embedModel` bge-m3, `minInstalls` 0, `filterDuplicates` true, `depsCheck` true) with projection defaults in `config.ts` + `defaultOrchestratorConfig`; `skillPriming.router` now supports `registry` (builds `omo_skill_find/get` directive).
